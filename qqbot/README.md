...
- `characters/玛修.json` —— 一张可直接参考的角色卡示例（已脱敏：QQ号与 token 是占位符，用前替换成你自己的）。
- `memory/玛修/users/123456789.json`、`memory/玛修/groups/987654321.json` —— 长期记忆文件的格式示例（内容为虚构）。

> 这些示例帮助你了解角色卡与记忆文件的结构。你自己的角色卡、真实记忆、密钥都不应提交到公开仓库。
