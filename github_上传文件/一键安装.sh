...
async def _send_reply(ws, reply, sender_func, target, state, speaker_id=None, speaker_name=""):
    max_bubbles = CARD.get("max_bubbles", 3)
    ...
    # 文字最多 max_bubbles 条；表情是纯附加，单独计数，不占文字栏位
    for bubble in (b.strip() for b in reply.split("\n") if b.strip()):
        tags = MEME_TAG_RE.findall(bubble)
        text = MEME_TAG_RE.sub("", bubble).strip()
        if text and sent < max_bubbles:
            await bubble_delay(text, first=(sent == 0))
